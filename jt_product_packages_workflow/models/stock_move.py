...
class StockMoveLine(models.Model):
    ...
    def write(self, vals):
        ...
        if updates or 'qty_done' in vals:
            moves = self.filtered(lambda ml: ml.move_id.state ==
                                  'done' or ml.move_id.picking_id and ml.move_id.picking_id.immediate_transfer).mapped('move_id')
            for move in moves:
            ...
            for move in self.filtered(lambda m: m.state in ['confirmed', 'waiting', 'partially_available']):
                ...
                if move._should_bypass_reservation():
                    ...
                    if not move.move_orig_ids:
                        ...
                        # could be moves to consider in what our siblings
                        # already took.
                        ...
                        for (location_id, lot_id, package_id, owner_id), quantity in available_move_lines.items():
                            ...
                            # this case `quantity` is directly the quantity on
                            # the quants themselves.
                            available_quantity = self.env['stock.quant']._get_available_quantity(
